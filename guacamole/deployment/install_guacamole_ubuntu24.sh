...
echo "Installing FreeRDP from source..."
cd /tmp
git clone --depth 1 --branch 2.11.7 https://github.com/FreeRDP/FreeRDP.git || true
cd FreeRDP
sudo apt install -y cmake libx11-dev libxkbfile-dev libxcursor-dev libxdamage-dev libxext-dev \
    libxi-dev libxinerama-dev libxrandr-dev libxrender-dev libxss-dev libxv-dev \
    libxfixes-dev libxft-dev libxxf86vm-dev libasound2-dev libcups2-dev libfuse3-dev
...
# Create symbolic link for Tomcat10
sudo ln -sf /var/lib/guacamole/guacamole.war /var/lib/tomcat10/webapps/guacamole.war
...
# Link configurations for Tomcat10
sudo ln -sf /etc/guacamole /usr/share/tomcat10/.guacamole
...
sudo systemctl status tomcat10 --no-pager -l | head -5
